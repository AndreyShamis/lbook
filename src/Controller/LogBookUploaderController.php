...
//    }
    ...
    protected function parseFile(string $filePath, LogBookTest $test, LogBookUpload $uploadObj, LoggerInterface $logger, bool $search_test_name = true, bool $search_test_verdict = true, bool $insertTests = true): array
    {
        ...
        $testVerdict = $this->parseVerdict('UNKNOWN');
                ...
                if ($search_test_verdict) {
                    if ($msgType_str === 'INFO') {
                        ...
                        if (\count($possibleVerdict) === 2) {
                            $possibleVerdictTmp = $this->parseVerdict($possibleVerdict[1]);
                            if ($possibleVerdictTmp->getName() !== 'UNKNOWN' && $testVerdict->getName() === 'UNKNOWN') {
                                $testVerdict = $this->parseVerdict($possibleVerdictTmp);
                            }
                            if ($testVerdict !== null) {
