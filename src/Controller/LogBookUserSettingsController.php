...
class LogBookUserSettingsController extends Controller
{
    ...
    /**
     * @Route("/new", name="user_settings_new", methods="GET|POST")
     * @param Request $request
     * @return Response
     */
    public function new(Request $request): Response
    {
        $logBookUserSetting = new LogBookUserSettings();
        $form = $this->createForm(LogBookUserSettingsType::class, $logBookUserSetting);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($logBookUserSetting);
            $em->flush();

            return $this->redirectToRoute('user_settings_index');
        }

        return $this->render('lbook/user_settings/new.html.twig', [
    ...
    /**
     * @Route("/{id}", name="user_settings_delete", methods="DELETE")
     * @param Request $request
     * @param LogBookUserSettings $logBookUserSetting
     * @return Response
     */
    public function delete(Request $request, LogBookUserSettings $logBookUserSetting): Response
    {
        if ($this->isCsrfTokenValid('delete'.$logBookUserSetting->getId(), $request->request->get('_token'))) {
            $em = $this->getDoctrine()->getManager();
            $em->remove($logBookUserSetting);
            $em->flush();
        }
