...
class LogBookCycleRepository extends ServiceEntityRepository
{
    ...
    public function delete(LogBookCycle $cycle): void
    {
        ...
        try {
            ...
            $c_path = $cycle->getLogFilesPath();
            /** This validation required in case that cycle path is /SOME_NUMBER only */
            if ($setup_path !== '' && mb_strlen($c_path) > mb_strlen($setup_path) && mb_strpos($c_path, $setup_path) !== false) {
            ...
            $this->logger->critical('[CYCLE][DELETE]: Throwable CYCLE ID:' . $cycle->getId(), array($ex->getMessage(), $ex, $c_path, $setup_path));
        }
