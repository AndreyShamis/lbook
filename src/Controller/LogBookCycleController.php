...
//                    if ($this->endsWith($md_key, '_SHOW') && !\in_array($md_key, $additional_cols, true)) {
//                        $additional_cols[] = $md_key;
//                    } else if ($this->endsWith($md_key, '_SHOW_OPT') && !\in_array($md_key, $additional_opt_cols, true)) {
//                        $additional_opt_cols[] = $md_key;
//                    }
                    $val[$md_key.'_MD'] = $md_val;
                }
                unset($val['meta_data']);
                $res[$key] = $val;
            }
     ...
     * @param LogBookCycle $cycle
     * @param int $page
     * @param bool $forJosn if True the JSON table for tests will be used
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function show(PagePaginator $pagePaginator, LogBookTestRepository $testRepo, LogBookCycle $cycle = null, $page = 1, $forJosn=false): ?Response
    {
        try {
            if (!$cycle) {
                throw new \RuntimeException('');
            ...
            if ($forJosn) {
                return $this->render('lbook/cycle/show.full.test_in_json.html.twig', $ret_arr);
            }
